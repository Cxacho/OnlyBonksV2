...
public class SlashAndCash : Card
{
    ...
    private void Start()
    {
        desc = $"Deal <color=white>{attack.ToString()}</color> damage, if you're slash will be deadly gain 10 gold";
    ...
    private void FixedUpdate()
    {
        ...
        if (attack == defaultattack)
            ...
            desc = $"Deal <color=red>{attack.ToString()}</color> damage, if you're slash will be deadly gain 10 gold";
        else
            desc = $"Deal <color=green>{attack.ToString()}</color> damage, if you're slash will be deadly gain 10 gold";
        this.transform.GetChild(0).GetChild(0).GetComponent<TextMeshProUGUI>().text = desc;
    }
        ...
        if (gameplayManager.canPlayCards == true)
        {
            ...
            foreach (Enemy en in _enemies)
            {
                if (en.targeted == true)
                {
                    ...
                    if((en._currentHealth - attack) <= 0)
                    {
                        gameplayManager.gold += 10;
                        gameplayManager.goldTxtTopUI.GetComponent<TextMeshProUGUI>().text = (gameplayManager.gold + 10).ToString();
                    }
